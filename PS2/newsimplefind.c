...
static void print_ls_line(const char *path, const char *name, const struct stat *sb, int is_symlink) {
    ...
    if (pw) printf("%s ", pw->pw_name); else printf("%ju ", (uintmax_t)sb->st_uid);
    if (gr) printf("%s ", gr->gr_name); else printf("%ju ", (uintmax_t)sb->st_gid);
    ...
    while ((entry = readdir(dp)) != NULL) {
        ...
        if (S_ISDIR(sb.st_mode)) {
            if (flag_xdev && start_dev != (dev_t)-1 && sb.st_dev != start_dev) {
                // print the directory (already done above if it matched) but do not recurse into it
                continue;
            }
...
int main(int argc, char *argv[]) {
    ...
    // Capture starting device (for -x) and stat the start path
    struct stat sb;
    ...
    if (strcmp(startbuf, "/") == 0) {
        ...
        const char *p = strrchr(startbuf, '/');
        leaf = (p ? p + 1 : startbuf);
        // If path ends with '/', fallback to the entire string (rare; usually normalized already)
        if (*leaf == '\0') leaf = startbuf;
    }

    // Print the starting node itself if it matches the -n filter (or no filter)
    visit_node("", leaf, startbuf, &sb);

    // Recurse if starting path is a directory
    if (S_ISDIR(sb.st_mode)) {
